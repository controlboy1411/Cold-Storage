...
import { Header, MenuTab, DashboardDetail, Footer, AboutUs, MenuTabMobile } from '../../components';
    ...
    return (
        <ThemeProvider theme={createTheme()}>
            <Box sx={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'auto' }}>
                ...
                <div>
                    <div className='report-about-container' style={{display: appContext.showButtonAbousUs ? 'flex' : 'none'}} onClick={() => setShowAboutUs(true)}>
                        <InfoOutlinedIcon sx={{color: '#074E9F'}}/>
                        <span className='report-about-title'>{t('about')}</span>
                    </div>
                </div>
                <Footer />
            </Box>
